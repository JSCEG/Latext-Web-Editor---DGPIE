...
export const fetchSpreadsheet = async (spreadsheetId: string, token: string): Promise<Spreadsheet> => {
  ...
  const response = await fetch(`${BASE_URL}/${spreadsheetId}?includeGridData=true`, {
...
export const appendRow = async (
  ...
  const response = await fetch(`${BASE_URL}/${spreadsheetId}/values/${range}:append?valueInputOption=USER_ENTERED&insertDataOption=INSERT_ROWS`, {
    method: 'POST',
    headers: getHeaders(token),
    body: JSON.stringify({
      range,
